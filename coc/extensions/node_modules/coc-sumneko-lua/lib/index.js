...
var Ctx = class {
  ...
  resolveBin() {
    ...
    if (!import_executable.default.sync(bin)) {
      import_coc4.window.showInformationMessage(`${bin} is not executable`, "error");
      return;
    }
    const args = ["-E", import_path3.default.join(serverDir, "bin", "main.lua"), `--locale=${this.config.locale}`].concat(import_coc4.workspace.getConfiguration("Lua").get("misc.parameters"));
    ...
    if (this.config.prompt) {
      ...
      if (ret === 0) {
        ...
        try {
          ...
          console.error(e);
          import_coc4.window.showInformationMessage("Upgrade server failed", "error");
          return;
        }
        this.client.start();
      } else {
        import_coc4.window.showInformationMessage(`You can run ':CocCommand sumneko-lua.install' to upgrade server manually`);
      }
    } else {
      import_coc4.window.showInformationMessage(`${msg} Run :CocCommand sumneko-lua.install to upgrade`);
    }
  }
  createClient() {
    const bin = this.resolveBin();
      ...
      middleware: {
        workspace: {
          configuration: async (params, token, next) => {
            ...
            const runtime = await import_coc4.workspace.nvim.call("expand", ["$VIMRUNTIME/lua"]);
    ...
    return new import_coc4.LanguageClient("sumneko-lua", "Sumneko Lua Language Server", serverOptions, clientOptions);
  }
  async startServer() {
  ...
  activateStatusBar() {
    const bar = import_coc4.window.createStatusBarItem();
  ...
  activateCommand() {
    this.client.onNotification("$/command", (params) => {
      ...
      for (const data of params.data) {
        const config = import_coc4.workspace.getConfiguration(void 0, data.uri);
        if (data.action == "add") {
...
async function activate(context) {
  ...
  if (!bin) {
    ...
    if (ctx.config.prompt) {
      ret = await import_coc5.window.showInformationMessage("Sumneko lua language server is not found, install now?", "Yes", "Cancel") || "Yes";
    }
    if (ret == "Yes") {
      try {
        ...
        console.error(e);
        import_coc5.window.showInformationMessage("Download Sumneko lua language server failed", "error");
        return;
      }
    } else {
      import_coc5.window.showInformationMessage(`You can run ':CocCommand sumneko-lua.install' to install server manually or provide setting 'serverDir'`);
      return;
    }
  }
  ...
  ctx.registerCommand("restart", (ctx2) => {
    return async () => {
      import_coc5.window.showInformationMessage(`Reloading sumneko lua-language-server...`);
      ...
      import_coc5.window.showInformationMessage(`Reloaded sumneko lua-language-server`);
    };
  });

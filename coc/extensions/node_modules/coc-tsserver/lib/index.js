...
function fromProtocolCallHierarchyItem(item) {
  ...
  const detail = useFileName ? import_path6.default.relative(import_coc7.workspace.cwd, import_path6.default.dirname(item.file)) : (_a = item.containerName) != null ? _a : "";
  const result = {
    name,
    detail,
...
function convertCompletionEntry(tsEntry, uri, position, context) {
  ...
  if (tsEntry.replacementSpan) {
    ...
    if (context.isMemberCompletion && context.line.endsWith("#")) {
      ...
      textEdit = import_coc9.TextEdit.replace(range, (_b = tsEntry.insertText) != null ? _b : label);
    }
  }
...
var getInstanceTempDir = (() => {
  let dir;
  return () => {
    dir != null ? dir : dir = path9.join(getRootTempDir(), makeRandomHexString(20));
    if (!fs4.existsSync(dir)) {
      fs4.mkdirSync(dir);
    }
    return dir;
